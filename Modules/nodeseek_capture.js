...
export default async function (ctx) {
  ...
  if (!hasSession && !hasCfClearance) return;
  ...
  try {
    ...
    if (patchResp.status === 200) {
      console.log('[NodeSeek] 加密同步成功 ✅');
      ctx.notify({ title: 'NodeSeek Cookie 已更新', body: '已加密同步到 Gist', sound: false });
    } else {
      ctx.notify({ title: 'NodeSeek 同步失败', body: `HTTP ${patchResp.status}` });
    }
  } catch (e) {
    ctx.notify({ title: 'NodeSeek 同步异常', body: e.message });
  }
}
